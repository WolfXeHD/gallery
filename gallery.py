...
parser.add_argument('--verbose',
                    '-v',
                    action='store_true',
                    help='print some info to the screen')
parser.add_argument(
    '--title-size',
    default='100',
    help=
    'scale the text size of the plot titles, expressed as a percentage [0-100]'
)
...
def ProcessDir(indir, is_parent=True, subdirs=[]):
    ...
    other_files = dict((k, v) for k, v in all_base_files.items()
                       if '.png' not in v and k.startswith('.') is False)
    ...
    for f, all_exts in sorted(base_files.iteritems()):
        ...
        if '.json' in all_exts:
            ...
            if len(json_data) > 0:
                tags = ' ' + ' '.join(
                    [CleanStr(x) for x in json_data.values()])
        ...
        if args.verbose:
            print info
        elements += PLOT_ELE.format(file='%s.png' % f,
                                    tags=tags,
                                    extra=extra,
    ...
    if len(groups) > 0:
        for key, val in groups.iteritems():
            buttons = '\n'.join([
                BUTTON.format(NAME=v, FILTER=CleanStr(v)) for v in sorted(val)
            ])
            button_groups += BUTTON_GROUP.format(GROUP=CleanStr(key),
                                                 TITLE=key,
                                                 BUTTONS=buttons)
        ...
        for f, all_exts in sorted(other_files.iteritems()):
            for ext in all_exts:
                ...
                if ext == '.md':
                    ...
                    other_html.append(
                        '  <li><a href="{FILEHTML}">{FILE}</a> <a href="{FILE}">[raw]</a></li>'
                        .format(FILEHTML=f + '.html', FILE=f + ext))
                else:
                    other_html.append(
                        '  <li><a href="{FILE}">{FILE}</a></li>'.format(
                            FILE=f + ext))
        if added > 0:
    ...
    if not is_parent or len(subdirs) > 0:
        ...
        for i, subdir in enumerate(subdirs):
            if i != 0: links += ' | '
            links += ' <a href="{SUBDIR}/index.html">{SUBDIR}</a>'.format(
                SUBDIR=subdir)
        links += '</p>'
